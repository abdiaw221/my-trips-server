...
public class EntryController {
  ...
  @Autowired private AuthUserInfoProviderURLFactory authUserInfoProviderURLFactory;
  ...
  public ResponseEntity<Response> authenticateUser(@RequestBody SignInRequest loginRequest) {
    log.info("Function={authenticateUser}, loginRequest={}", loginRequest);
    return Response.getResponseEntity(
        true, "You are now logged in.", userService.signIn(loginRequest));
  }
  ...
  public ResponseEntity<Response> createUser(@RequestBody SignUpRequest signUpRequest) {
    ...
    return Response.getResponseEntity(
        true, "User Created. Please check your email address and verify your account");
  }
  ...
  public ResponseEntity<Response> updateBannedStatusManually(
      @RequestBody UpdateBannedStatusRequest request) {
  ...
  @Operation(
      summary = "Update verify status by admin manually",
  ...
  public ResponseEntity<Response> updateVerifyStatusManually(
      @RequestBody UpdateVerifyStatusRequest request) {
  ...
  public ResponseEntity<Response> getAllPaginatedUsers(
    ...
    PaginationArgs paginationArgs =
        new PaginationArgs(pageNo, pageSize, sortBy, ascOrDesc, parameters);

    return Response.getResponseEntity(
        true, "All paginated users loaded", userService.getAllPaginatedUser(paginationArgs));
  }
  ...
  public ResponseEntity<Response> getUserInfo() {
    return Response.getResponseEntity(
        true, "User info loaded", userService.getUserInfo(userService));
  }
  ...
  public ResponseEntity<Response> findUserById(@PathVariable("userId") long userId) {
    return Response.getResponseEntity(
        true, "User info loaded", userService.findByIdWithException(userId));
  }
  ...
  public ResponseEntity<Response> updateUserInfo(
    ...
    return Response.getResponseEntity(
        true, "User updated successfully!", userService.saveUser(user));
  }
  ...
  public ResponseEntity<Response> createAdminUser(@RequestBody AdminCreationRequest request) {
    return Response.getResponseEntity(
        true, "Admin user created successfully", userService.createAdminUser(request));
  }
  ...
  public ResponseEntity<Response> updateUserInfo(@Valid @RequestBody UpdateUserFromAdmin request) {
    return Response.getResponseEntity(
        true, "User updated successfully", userService.updateUserFromAdmin(request));
  }
  ...
  public ResponseEntity<Response> changePassword(@RequestBody ChangePasswordRequest request) {
    return Response.getResponseEntity(
        true, "Password Changed successfully", userService.changePassword(request));
  }
  ...
  public ResponseEntity<Response> requestForgetPassword(
      @RequestBody InitialForgetPasswordRequest initialForgetPasswordRequest) {
    ...
    if (AppUtils.isValidMail(user.getEmail())) {
      ...
      if (appProperties.getActiveProfile() != AppConstants.environment.development) {
        mailService.sendMail(
            user.getEmail(),
            AppConstants.forgetPasswordSubject,
            AppConstants.forgetPasswordText
                + appProperties.getBackEndUrl()
                + AppConstants.RESET_PASSWORD_SUBURL
                + token);
      }

      return Response.getResponseEntity(
          true, "Reset password link sent to your registered email address.");
    } else {
      return Response.getResponseEntity(
          HttpStatus.EXPECTATION_FAILED, "Your email format is not correct.");
    }
  }
    ...
    return refreshTokenService
        ...
        .map(
            userId -> {
              ...
              return Response.getResponseEntity(
                  true, "Token Successfully Generated.", newTokenResponse);
            })
        .orElseThrow(
            () ->
                new ResponseException(requestRefreshToken + " Refresh token is not in database!"));
  }
  ...
  public ResponseEntity<Response> socialLogin(
    ...
    if (provider == SocialAuthType.GOOGLE) {
      ...
      builder =
          UriComponentsBuilder.fromHttpUrl(authUserInfoProviderURLFactory.getUserInfoUri(provider));
      ...
      builder =
          UriComponentsBuilder.fromHttpUrl(authUserInfoProviderURLFactory.getUserInfoUri(provider))
              .queryParam(
                  "fields",
                  "id,first_name,middle_name,last_name,name,email,verified,picture.width(250).height(250)")
    ...
    ResponseEntity<Map> response =
        restTemplate.exchange(builder.toUriString(), HttpMethod.GET, entity, Map.class);
    ...
    OAuth2UserInfo oAuth2UserInfo =
        OAuth2UserInfoFactory.getOAuth2UserInfo(provider, (Map) response.getBody());
    ...
    if (user.getBanned()) {
      return Response.getResponseEntity(
          ...
          "You are banned by admin. If you think it is a mistake then please contact with us.");
    }
